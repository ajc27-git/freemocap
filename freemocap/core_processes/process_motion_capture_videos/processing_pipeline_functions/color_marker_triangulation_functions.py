...
def get_color_markers_triangulated_data(
    ...
    if color_markers_2d_data is None:
        # Try to load from disk if passed data is None (e.g. tracking was skipped)
        path_to_2d_data = Path(processing_parameters.recording_info_model.raw_data_folder_path) / COLOR_MARKERS_2D_NPY_FILE_NAME
        if path_to_2d_data.exists():
            logger.info(f"Loading 2D color marker data from {path_to_2d_data}")
            color_markers_2d_data = np.load(path_to_2d_data)
        else:
    ...
    # Get enabled marker data (name and color)
    marker_data_list = [
        {
            "name": config.marker_name,
            "color": list(config.target_color_bgr[::-1])  # Convert BGR to RGB
        }
        for config in processing_parameters.color_tracker_parameters_model.marker_configs 
        if config.enabled
    ]
    ...
    if color_markers_2d_data.shape[0] == 1:
        ...
        # Use the local single camera processing function
        color_markers_3d_data = process_single_camera_color_markers_data(
...
def process_single_camera_color_markers_data(
        ...
        marker_data_list: List[Dict] = None,
) -> np.ndarray:
    """
...
def save_color_markers_3d_data(
    ...
    if marker_data_list:
        names_path = raw_data_folder_path / "color_markers_names.json"
        with open(names_path, "w") as f:
            json.dump(marker_data_list, f, indent=4)
        logger.info(f"Saved color marker info to: {names_path}")